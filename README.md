...
The input file path must point to the directory containg the ``meanQ`` files generated by the structure analysis. The ``names.txt`` and ``groups.txt`` input files are similar to the popmap input files. The value of 10 in the command is the total number of values tested for K.
